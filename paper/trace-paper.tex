...
\hypersetup{
  hypertexnames=false,
  pdftitle={TRACE: Structural Planning, Tool-Grounded Multi-Agent Verification, and Traceability-Guided Local Repair for LLM-Based Java Unit Testing},
  pdfauthor={Nguyen Tuan Dung; Vuong Kieu Anh; Doan Huu Minh Quang; Ngo Duc Chinh; Tran Van Han},
  pdfsubject={LLM-based Java unit testing},
  pdfkeywords={unit test generation, large language models, structural planning, traceability-guided repair, multi-agent verification, Java}
}
...
\author[1]{\sur{Nguyen} \fnm{Tuan Dung}}\email{dungnthe180616@fpt.edu.vn}
\author[1]{\sur{Vuong} \fnm{Kieu Anh}}\email{anhvkhe180546@fpt.edu.vn}
\author[1]{\sur{Doan} \fnm{Huu Minh Quang}}\email{quangdhmhe181540@fpt.edu.vn}
\author[1]{\sur{Ngo} \fnm{Duc Chinh}}\email{chinhndhe180889@fpt.edu.vn}
\author[1]{\sur{Tran} \fnm{Van Han}}\email{hantvhe181281@fpt.edu.vn}
\affil[1]{\orgname{FPT University}, \orgaddress{\city{Hanoi}, \country{Vietnam}}}

\abstract{Large language model (LLM)-based Java unit-test generation remains limited by weak structural planning, uncontrolled repair, and unreliable verification. TRACE combines auditable scenario planning, traceability-guided local repair, and tool-grounded multi-agent verification. We evaluated these components in three studies and compared the integrated pipeline with EvoSuite. In the planning study, 600 runs on 150 SF110 methods and two model configurations showed that planning increased aggregate execution success from 58.00\% to 88.33\%, while using approximately twice as many tokens. Structural metrics improved after Holm correction, but differing non-missing denominators limit the branch-coverage comparison. In the repair study, scenario-local repair succeeded on 60\% of 100 defect units, versus 46\% for method-level repair and 76\% for full-class regeneration; its advantage over method-level repair was not significant after correction. In the exploratory verification study, Gemini false acceptance was 26.92\% for the specialized verifier versus 57.69\% for a general reviewer, and repair-routing accuracy was 66.67\% versus 42.59\%, at higher inference cost. Across 450 paired runs, TRACE's integrated pipeline delivered verified suites in 55.11\% of cases versus 28.89\% for EvoSuite ($p=0.0001$). This comparison used a TRACE-specific verification gate whose neutrality across generators remains unestablished. The results show improved delivered reliability in the evaluated settings, alongside cost and validity limits.}
...
The literature reviewed here leaves three specific gaps. First, standardized test design techniques such as equivalence partitioning, boundary-value analysis, and structural coverage provide auditable rules to infer test conditions \cite{bib30}, while prior LLM-based work has explored method slicing and testing-intention prompts \cite{bib6,bib7}. Therefore, the performance of a package that adds structural planning before generation remains unclear when the LLM, context, test-method cap, and generation budget are fixed, but the added planning call and its compute are retained. Second, ChatTester, ChatUniTest, TestART, and TestLoter use feedback to fix test failures \cite{bib26,bib4,bib27,bib28}; among the approaches reviewed here, we did not identify published documentation of an immutable bidirectional contract between the atomic scenario and the test method. The benefits of scenario-local repair in terms of cost, edit scope, repair iterations, and regression therefore need to be measured directly against fixing a failing test method without scenario context and regenerating the entire class. Third, MAGISTER separates the Analyzer, Test Generation, Executor, and Refiner Agent, while CANDOR uses multi-agent coordination for oracle generation \cite{bib31,bib22}. Among the approaches reviewed in this study, we did not identify a framework that jointly separates provenance-linked verification evidence streams, preserves immutable scenario identity, and aggregates them into a primary failure cause and repair route at the scenario level. The performance of this specialized-verification package needs to be distinguished from Basic Tool Gates and a General-Purpose Reviewer provided with the same evidence.
...
Automated unit test generation has evolved from search-based and random-generation techniques such as EvoSuite \cite{bib1} and Randoop \cite{bib2} to LLM-based approaches, although industrial evaluations indicate that the assertion quality of the previous tools is still limited \cite{bib3}. Early LLM methods used the focal method as the main context: ChatUniTest \cite{bib4} applies a Generation--Validation--Repair process, while TestPilot \cite{bib5} uses re-prompting based on execution errors, though it has only been evaluated on JavaScript/npm. Subsequent studies expanded the context and testing intentions: HITS \cite{bib6} breaks the focal method into code slices, Test Intention \cite{bib7} incorporates testing intention into the prompt, and CATGen \cite{bib29} incorporates project dependencies and structured context. Overall, these studies show the value of selecting context and testing intentions, but the literature reviewed here does not establish the individual impact of a structured planning phase separate from code generation under equivalent controlled conditions.

\subsection{Test Traceability and Automated Repair}\label{subsec:rw-traceability}

TRACETS4J \cite{bib10} builds test--focal-method links at the data level across large Java project sets. Compilation/execution feedback has supported test repair in ChatUniTest \cite{bib4}, TestPilot \cite{bib5}, and TestART \cite{bib27}. However, the scope of modifications and the impact of each change on previously verified tests still need to be controlled, especially when multiple tests share fixtures or dependencies. tsDetect \cite{bib25} adds maintenance signals through test smell detection but does not replace evidence of assertion correctness.

\subsection{Tool-Grounded Verification and Agent Architectures}\label{subsec:rw-verification}

SymPrompt \cite{bib11} and CoverUp \cite{bib12} use execution paths and coverage feedback to guide test generation, although high coverage does not guarantee strong assertions. Mutation testing provides this additional signal: MUTGEN \cite{bib13}, ACH \cite{bib14}, AdverTest \cite{bib15} improve the ability to kill mutants, while TOGLL \cite{bib16} evaluates oracles in terms of correctness and fault detection strength---showing that coverage, mutation, and oracle quality are signals that cannot completely replace each other. At the actor architecture level, MetaGPT \cite{bib17} and SWE-agent \cite{bib18} establish role-playing patterns and tool interactions; SGAgent \cite{bib20} combines a localizer, suggester, and fixer with a repository-level knowledge graph. In test generation, Retry and Refine \cite{bib21} use seven agents but only evaluate on MBPP/HumanEval; CANDOR \cite{bib22} focuses on improving the oracle through multi-agent coordination; MAGISTER \cite{bib31} separates analyzer, generator, executor, and refiner roles. These works demonstrate the potential of coordinating roles and tool feedback, but the literature reviewed here does not isolate the value of a specialized verifier given the same evidence compared with a simpler verification mechanism. A surviving mutant, for example, does not automatically prove the oracle is weak if the relationship with the focal method and the corresponding test objective has not been established.
...
Plan \ensuremath{\Pi} remains unchanged after release. If a step fails due to test implementation, TRACE fixes the implementation test method $T_i$ rather than silently changing the scenario set. If evidence shows that the expected behavior is unfounded, oracle sources are conflicting, or the structural target is infeasible, the scenario is moved to the abstained/unresolved state with provenance instead of being corrected like a test defect. Therefore, the plan can be audited by counting and comparing equivalence classes, boundary values, branch outcomes, and exception paths before the test code is accepted. In contrast to methods that use code slices or testing-intention prompts \cite{bib6,bib7}, the planning unit of TRACE is the structural target with oracle provenance and can be compared with Direct Generation under the same generation budget in RQ1.
...
The integrated TRACE--EvoSuite evaluation used 150 frozen SF110 targets with three fixed repetitions per target, yielding 450 paired observations under a common evaluator and treatment-blinded B2 gate. It measured delivered-system outcomes rather than isolating the causal effect of any one component; its design and results are reported in Section~\ref{subsec:trace-evosuite}.
...
RQ2 uses core repair success, token/API cost, number of repair rounds, Changed LOC, and regression-producing repair attempts. A core repair is considered successful if it is applied, compiles, makes the focal test pass on the fixed revision, exhibits bug-revealing behavior on the buggy revision if needed, and does not leave any regression. Coverage and mutation are only reported descriptively on successful repairs with measurement evidence. Since the data does not yet have independent scenario conformance and structural-target satisfaction, RQ2 does not evaluate the entire acceptance contract of the framework. RQ3 uses decision accuracy, False-Acceptance Rate (false acceptances divided by the 26 false-passing cases), False-pass F1 with false-pass detection as the positive class, attribution Macro-F1 across cause labels, repair-route accuracy on the 54 \texttt{REPAIR\_REQUIRED} cases, decision coverage over all 73 cases, and selective accuracy among covered decisions. Provider-invalid or partially invalid outputs are retained through their case-level aggregate decisions and are also counted as output errors. \texttt{ABSTAIN} is a valid label for three-class decision accuracy and can be correct when the gold label is also \texttt{ABSTAIN}; it is excluded from decision coverage and selective accuracy, and it is neither an acceptance nor a repair prediction in the false-passing analysis. Reported abstention rates use all 73 cases as their denominator. Cost is measured by token usage and number of agent calls.

The McNemar exact test is used for binary outcomes and the Wilcoxon signed-rank test for paired continuous metrics where specified. RQ1 applies Holm correction within each model's five-metric primary family and reports matched odds ratios or paired rank-biserial correlations; its conditional analysis is explicitly unadjusted. RQ2 applies Holm correction to its prespecified ten-test family and reports the stated effect sizes and unadjusted paired-bootstrap intervals. RQ3 is exploratory: its McNemar tests and bootstrap intervals are unadjusted unless a table states otherwise. The integrated comparison uses its own prespecified Holm family and hierarchical bootstrap. Because each unit--treatment is mainly run only once and the data is clustered by project, the results are limited to the evaluated configurations.
...
CSR: Compilation Success Rate; ESR: Execution Success Rate; STS: Structural-Target Satisfaction; BC: focal-method Branch Coverage; MS: focal-method Mutation Score. CSR and ESR have N = 150 for each model--approach combination. STS, BC, and MS are averages over non-N/A observations. For Gemini, the number of STS/BC/MS observations is 140/139/150 for both approaches; for DeepSeek, the corresponding numbers are 148/148/150 in DIRECT and 141/140/150 in PLANNED. The available aggregate does not include case-level N/A reason codes, and the DeepSeek STS/BC denominators differ by treatment. Consequently, the STS/BC means---including the aggregate BC values highlighted below---are not common-cohort contrasts and may reflect differential missingness; they remain descriptive until reason-coded missingness, common-pair, and sensitivity analyses are available. The summary rows only provide aggregate descriptions and should not be used to infer or rank the two LLMs. Across both model configurations, PLANNED achieved a CSR of 96.33\%, 17.33 percentage points higher than DIRECT's 79.00\%. ESR increased from 58.00\% to 88.33\%, corresponding to 91 additional generated test suites executing successfully. For structural-adequacy metrics, STS increased descriptively from 35.52\% to 58.47\%, BC from 43.51\% to 70.58\%, and MS from 41.43\% to 63.92\%. These aggregated results show a consistent trend, but are only descriptive.
...
Among the two evaluated model configurations, PLANNED statistically significantly improves ESR, STS, BC, and MS after Holm correction; CSR only reaches statistical significance on DeepSeek. Primary results indicate that the PLANNED package is associated with improved pipeline reliability and end-to-end test adequacy. Secondary paired-executed analysis shows that adequacy benefits are still clear on Gemini, but weak and unstable on DeepSeek. Therefore, the data does not yet support a generalized conclusion that PLANNED always produces stronger executable test suites than DIRECT across all LLMs. The achieved benefits also come with nearly double the token usage and higher wall-clock time. Because PLANNED includes an additional planning call and greater total inference compute, this package-level ablation does not causally isolate the structural-plan representation from the extra call or tokens. Each configuration only has one run per focal method, the available aggregate lacks N/A reason codes, and the tests have not yet accounted for project clustering or between-run variability. Therefore, the conclusion is limited to the dataset, model configurations, and budget that were examined.
...
B0 uses compilation and selected-JUnit results. B1 is a general reviewer. B2 consists of three specialists in plan-to-test conformance, compilation/runtime, and coverage/mutation; a deterministic aggregator creates the decision, primary cause, and repair route. The aggregator first selects the highest-priority valid \texttt{DETECTED} finding in the fixed order \texttt{COMPILATION\_ERROR}, \texttt{SHARED\_INFRASTRUCTURE\_ERROR}, \texttt{SETUP\_OR\_RUNTIME\_ERROR}, \texttt{TARGET\_NOT\_REACHED}, \texttt{WEAK\_ORACLE}, \texttt{PLAN\_TEST\_MISMATCH}, and \texttt{ASSERTION\_ERROR}. If no such finding exists, it emits \texttt{VERIFIED} only when all three specialists return \texttt{CLEAR} with execution status \texttt{OK}; otherwise, including a partially invalid specialist set, it emits \texttt{ABSTAIN} with route \texttt{REQUEST\_TOOL\_EVIDENCE}. In each case, B1 and B2 receive an evidence envelope with the same SHA-256. The cohort has 26 false-passing cases. Its decision labels comprise 54 \texttt{REPAIR\_REQUIRED}, 8 \texttt{VERIFIED}, and 11 \texttt{ABSTAIN} cases.
...
\label{eq:rq3-far}
\end{equation}
Here, $y_i$ and $\hat{y}_i$ are the gold and predicted decisions in $\{\texttt{VERIFIED},\texttt{REPAIR\_REQUIRED},\texttt{ABSTAIN}\}$. Decision accuracy counts an exact decision-label match over all 73 cases, so a predicted \texttt{ABSTAIN} is correct for this metric when the gold decision is also \texttt{ABSTAIN}. With $c_i=\mathbb{I}(\hat{y}_i\neq\texttt{ABSTAIN})$, decision coverage and selective accuracy are
\begin{equation}
\begin{aligned}
...
\end{equation}
Attribution Macro-F1 is the unweighted mean of cause-level F1 scores over the seven gold defect classes \texttt{ASSERTION\_ERROR}, \texttt{COMPILATION\_ERROR}, \texttt{PLAN\_TEST\_MISMATCH}, \texttt{SETUP\_OR\_RUNTIME\_ERROR}, \texttt{SHARED\_INFRASTRUCTURE\_ERROR}, \texttt{TARGET\_NOT\_REACHED}, and \texttt{WEAK\_ORACLE}. Per-class precision, recall, and F1 are set to zero when their respective denominator is zero. Routing accuracy is the proportion of the 54 \texttt{REPAIR\_REQUIRED} cases for which the predicted route belongs to the gold case's acceptable-route set; the route vocabulary is \texttt{LOCAL\_TEST\_REPAIR}, \texttt{REGENERATE\_TEST}, \texttt{SHARED\_INFRASTRUCTURE\_REPAIR}, \texttt{REQUEST\_TOOL\_EVIDENCE}, and \texttt{NONE}. Thus, a case with multiple acceptable routes is correct when the prediction matches any listed route rather than only a canonical first route. The overall abstention rate uses all 73 cases. Invalid or partially invalid provider outputs were not removed from the GLM sensitivity cohort: their case-level aggregated decision or abstention remained in the denominator.
...
\subsection{Exploratory End-to-End TRACE System Evaluation against EvoSuite}\label{subsec:trace-evosuite}

\subsubsection{Comparison Design and Statistical Methods}\label{subsubsec:trace-design}

In this comparison, TRACE is the integrated pipeline combining structural planning, traceable generation, tool execution, B2 verification, and at most two TGSLR rounds. We compare TRACE with EvoSuite 1.2.0 \cite{bib1} on a frozen cohort of 150 focal targets from 58 SF110 projects, version SF110-20130704 \cite{bib37}. Each target was run with three fixed repetitions, 42, 43, and 44. The design yields 450 TRACE--EvoSuite pairs on the same target and repetition. Both treatments used the same Maven/Surefire, JaCoCo, and PIT evaluator and then passed through the same treatment-blinded B2 gate. The available campaign description does not establish that this TRACE-oriented gate is construct-neutral for EvoSuite. Each treatment had a recorded wall-clock limit of 900 seconds, but the available aggregate report does not specify whether that limit was applied per target--repetition--treatment or at another scope. TRACE had at most two TGSLR rounds, whereas EvoSuite had no repair round.
...
\subsubsection{Delivered-System ITT Results}\label{subsubsec:trace-itt}

Table~\ref{tab:trace-itt} reports the ITT outcomes for the two treatments.

\begin{table*}[htbp]
\caption{ITT results for TRACE and EvoSuite across 450 paired observations}\label{tab:trace-itt}
\centering
\scriptsize
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X r r r l r r@{}}
\toprule
Metric & TRACE & EvoSuite & Delta & 95\% CI for Delta & $p$ & $p_{\mathrm{Holm}}$ \\
\midrule
...
\end{tabularx}
\begin{tablenotes}
\item Delta = TRACE minus EvoSuite, in percentage points. CI denotes the percentile 95\% confidence interval from the hierarchical cluster bootstrap. The $p$ values are from the two-sided paired permutation test. VSR was not included in the prespecified Holm family.
\end{tablenotes}
\end{table*}

TRACE achieved a VSR of 55.11\% (248/450), higher than EvoSuite's 28.89\% (130/450), a difference of 26.22 percentage points. The 95\% confidence interval [14.22; 38.36] does not include 0, and the permutation test gave $p=0.0001$. Delivered CSR and delivered ESR had the same difference, with $p_{\mathrm{Holm}}=0.0005$. Within the evaluated cohort and TRACE-specific evaluation contract, these results show a higher probability that TRACE produced a suite that was delivered and then counted as compiled and executed after the verification gate; they do not compare raw pre-gate compilation or execution rates.
...
\subsubsection{Target-Level Paired Analysis}\label{subsubsec:trace-paired}

Among the 450 repetition-level pairs, only TRACE had a verified suite in 148 pairs, only EvoSuite had a verified suite in 30 pairs, both had a verified suite in 100 pairs, and neither had a verified suite in 172 pairs. After averaging the three repetitions for each target, the numbers of targets where TRACE won, EvoSuite won, and the treatments tied were 62/14/74 for VSR, 59/27/64 for branch coverage, and 60/19/71 for mutation score, respectively. The statistical analysis used these 150 target-level differences to avoid treating the three repetitions of the same target as independent observations.

\subsubsection{Candidate Diagnostics before the Verification Gate}\label{subsubsec:trace-candidate}

Table~\ref{tab:trace-candidate} reports candidate metrics before the delivered-system gate.

\begin{table*}[htbp]
\caption{Candidate metrics before applying the delivered-system gate}\label{tab:trace-candidate}
\centering
\scriptsize
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X r r r l@{}}
\toprule
Metric & TRACE & EvoSuite & Delta & 95\% CI for Delta \\
\midrule
...
TRACE had a higher candidate point estimate on all three metrics. The gap between candidate and delivered outcomes shows that some outputs had quality measurable by the tools but did not pass all generation, repair, and verification gates. Therefore, Table~\ref{tab:trace-candidate} is used to explain pipeline behavior, while the main conclusion rests on Table~\ref{tab:trace-itt}.

\subsubsection{Outcome Integrity and Cost}\label{subsubsec:trace-integrity}

TRACE had 248 runs verified by B2. The remaining terminal outcomes comprised 75 budget-exhausted, 52 repair-exhausted, 41 B2-abstention, 17 verifier-rejected, 12 tool-failure, and 5 provider-failure runs. EvoSuite had 130 verified runs; the remaining outcomes comprised 175 generation-failure, 139 baseline records labeled \texttt{repair\_exhausted} by the shared terminal-state taxonomy, and 6 unverified-abstention runs. This label does not imply that EvoSuite performed a repair round. All these states were retained in the ITT sample.

The recorded total duration was 19.65 hours for TRACE and 14.73 hours for EvoSuite, corresponding to means of 157.18 and 117.81 seconds per pair, respectively. TRACE used 37,564,632 provider tokens, averaging 83,477 tokens per pair. The duration totals are the sums of durations for each treatment, not the elapsed time of the entire campaign, and do not describe the degree of parallelism.

\subsubsection{Interpretation and Limits of the Conclusion}\label{subsubsec:trace-interpretation}

TRACE achieved higher delivered-system outcomes than EvoSuite with statistical significance on the reported ITT outcomes in the recorded campaign. This conclusion applies to the integrated pipeline under the fixed SF110 cohort, tool lock, budget, and TRACE-specific verifier. Treatment blinding does not by itself establish that the B2 gate is construct-neutral across the two generators. The comparison also does not isolate the causal effects of Structural Planning, TGSLR, or B2; those components must be interpreted through RQ1, RQ2, and RQ3, respectively. The results do not establish TRACE's superiority on every benchmark, LLM, budget, tool version, or evaluation contract.
...
In the separate integrated evaluation, TRACE and EvoSuite were compared on 450 target--repetition pairs from 150 SF110 targets. TRACE achieved a VSR of 55.11\%, compared with 28.89\% for EvoSuite; the 26.22-percentage-point difference was statistically significant in the paired permutation test ($p=0.0001$). The delivered-system ITT adequacy metrics also favored TRACE after Holm correction. Recorded treatment durations totaled 19.65 hours for TRACE and 14.73 hours for EvoSuite, while TRACE used 37,564,632 provider tokens; the corresponding EvoSuite provider-token total was not reported. This is an integrated delivered-system result under the fixed cohort, budget, toolchain, and TRACE-specific evaluation contract; it does not isolate any one TRACE component, establish gate neutrality, or establish superiority in other settings.

...
\abstract{Large language model (LLM)-based Java unit-test generation remains limited by weak structural planning, uncontrolled repair scope, and unreliable verification decisions. TRACE addresses these limitations through structure-derived atomic scenario planning, Traceability-Guided Scenario-Local Repair (TGSLR), and tool-grounded specialized multi-agent verification. The contributions were evaluated independently in three studies. In RQ1, 600 runs on 150 SF110 focal methods and two model configurations showed that planning increased aggregate execution success from 58.00\% to 88.33\% and end-to-end branch coverage from 43.51\% to 70.58\%. Execution success and three structural-quality metrics improved significantly after Holm correction in both configurations, whereas compilation success improved significantly only for DeepSeek. Conditional benefits among pairs that both executed were clear for Gemini but weak and unstable for DeepSeek, while planning used approximately 2.03 times as many tokens. In RQ2, TGSLR repaired 60\% of 100 LIBRO--Defects4J error units, compared with 46\% for local repair without scenario context and 76\% for full-class regeneration. Its advantage over local repair was not significant after Holm correction. In the exploratory RQ3 study of 73 cases from 14 projects, specialized verification reduced Gemini's false-acceptance rate from 57.69\% to 26.92\% and increased routing accuracy from 42.59\% to 66.67\%, while using 2.97 times as many input tokens as a general reviewer. These findings characterize the reliability, locality, and cost trade-offs of TRACE within the evaluated settings.}
...
FullChain achieved a VSR of 55.11\% (248/450), higher than EvoSuite's 28.89\% (130/450), a difference of 26.22 percentage points. The 95\% confidence interval [14.22; 38.36] does not include 0, and the permutation test gave $p=0.0001$. CSR and ESR had the same difference, with $p_{\mathrm{Holm}}=0.0005$. Thus, the results support H1: within the evaluated cohort and configurations, FullChain increased the probability of producing a suite that was delivered, compiled, and executed successfully.

All three adequacy metrics were also statistically significantly higher. STS increased by 18.45 percentage points, with a 95\% CI of [8.01; 29.91] and $p_{\mathrm{Holm}}=0.0006$. Branch coverage increased by 13.91 percentage points, with a 95\% CI of [3.20; 25.69] and $p_{\mathrm{Holm}}=0.0007$. Mutation score increased by 17.14 percentage points, with a 95\% CI of [6.83; 28.23] and $p_{\mathrm{Holm}}=0.0006$. All confidence intervals exclude 0 when all terminal outcomes are retained under ITT. The results therefore support H2 within the frozen cohort, budget, and tool configuration.
...
Reproducibility is supported by the paired-comparison CSV, raw records, and availability ledger of campaign \nolinkurl{official-150-20260920-r42-r44-v1}. The SHA-256 hashes of these files are stored in the campaign results. At publication, the cohort manifest, tool lock, redacted configuration, analysis script, and result artifacts must be released together.

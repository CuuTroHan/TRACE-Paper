...
For RQ2, TGSLR, FTMR, and FCR were evaluated on the same 100 initial failing artifacts across six LIBRO--Defects4J projects. The treatments used the same repository state, failure evidence, model, seed, repair budget, and stopping conditions. TGSLR received scenario--test mapping and only fixed the corresponding test method; FTMR fixed the same method but without scenario context; FCR was allowed to regenerate the entire test class. Each treatment ran once on each unit using the gemini 3.6 flash model, generating a total of 300 logic runs, with up to three repair rounds and a budget of 75,000 tokens per run. The budget is checked between completed calls, so the last call may cause the total tokens to exceed the threshold; provider retries are not counted as a new experimental unit.
...
This evaluation compares the complete FullChain pipeline with EvoSuite 1.2.0 on a frozen cohort of 150 focal targets from 58 SF110 projects, version SF110-20130704. Each target was run with three fixed repetitions, 42, 43, and 44. The design yields 450 FullChain--EvoSuite pairs on the same target and repetition. Both treatments used the same Maven/Surefire, JaCoCo, and PIT evaluator and then passed through the same treatment-blinded B2 gate. Each treatment had a global wall-clock limit of 900 seconds; FullChain had at most two TGSLR rounds, whereas EvoSuite had no repair round.
...
CSR & 55.11\% & 28.89\% & +26.22 & [14.22; 38.36] & 0.0001 & 0.0005 \\
ESR & 55.11\% & 28.89\% & +26.22 & [14.22; 38.36] & 0.0001 & 0.0005 \\
...
FullChain achieved a VSR of 55.11\% (248/450), higher than EvoSuite's 28.89\% (130/450), a difference of 26.22 percentage points. The 95\% confidence interval [14.22; 38.36] does not include 0, and the permutation test gave $p=0.0001$. CSR and ESR had the same difference, with $p_{\mathrm{Holm}}=0.0005$. Within the evaluated cohort and configurations, these results support the conclusion that FullChain increased the probability of producing a suite that was delivered, compiled, and executed successfully.
...
The results support the conclusion that FullChain outperformed EvoSuite with statistical significance on the prespecified ITT outcomes in the Official campaign. This conclusion applies to the integrated pipeline under the fixed SF110 cohort, tool lock, budget, and verifier. It does not isolate the causal effects of Structural Planning, TGSLR, or B2; those components must be interpreted through RQ1, RQ2, and RQ3, respectively. The results also do not establish FullChain's superiority on every benchmark, LLM, budget, or tool version.

The campaign identifier is:
\begin{center}
\small\nolinkurl{official-150-20260920-r42-r44-v1}
\end{center}
Its paired-comparison CSV, raw records, and availability ledger support reproducibility. The SHA-256 hashes of these files are stored in the campaign results. At publication, the cohort manifest, tool lock, redacted configuration, analysis script, and result artifacts must be released together.
...
In the separate integrated evaluation, FullChain and EvoSuite were compared on 450 target--repetition pairs from 150 SF110 targets. FullChain achieved a VSR of 55.11\%, compared with 28.89\% for EvoSuite; the 26.22-percentage-point difference was statistically significant in the paired permutation test ($p=0.0001$). The ITT adequacy metrics also favored FullChain after Holm correction. Recorded treatment durations totaled 19.65 hours for FullChain and 14.73 hours for EvoSuite, while FullChain used 37,564,632 provider tokens; the corresponding EvoSuite provider-token total was not reported. This result applies to the fixed cohort, budget, toolchain, and verifier and does not isolate the effect of any one TRACE component or establish superiority in other settings.
